...
export const FavouriteManga = ({rendered}) => {
    const {favouriteManga, isSearch, searchResults} = useGlobalContext();
    
    const navigate = useNavigate();

    const conditionalRender = () => {
        if(!isSearch && rendered === 'favourite') {
            return favouriteManga?.map((manga) => {
                return (
                    <div key={manga.mal_id} className="relative h-[500px] border-[5px] bg-[#e5e7eb] border-[#e5e7eb] pb-[64px] rounded-[7px] overflow-hidden">
                        ...
                        <div
                            ...
                            onClick={() => navigate(`/manga/${manga.mal_id}`)} // Click vào overlay sẽ điều hướng
                        >
                        ...
                        <div className="modal" role="dialog">
                            ...
                            <label className="modal-backdrop text-[50px]" htmlFor={`modal_${manga.mal_id}`}>Đóng</label>
                        </div>
                    </div>
                );
                
                // <Link className="relative h-[500px] border-[5px] bg-[#e5e7eb] border-[#e5e7eb] pb-[64px] rounded-[7px] overflow-hidden" to={`/manga/${manga.mal_id}`} key={manga.mal_id}>
                //     <img className="w-full h-full object-cover rounded-[5px]" src={manga.images.jpg.large_image_url} alt="" />
                //     <div className="text-center font-[600] text-[18px] mt-[8px] mx-[4px] line-clamp-2">{manga.title}</div>
