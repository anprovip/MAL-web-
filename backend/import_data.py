    ...
    try:
        ...
        for idx, row in df.iterrows():
            ...
            anime = Anime(
                ...
                scored_by=row['scored_by'] if 'scored_by' in row and not pd.isna(row['scored_by']) else None,
                rank=row['rank'] if 'rank' in row and not pd.isna(row['rank']) else None,
                popularity=row['popularity'] if 'popularity' in row and not pd.isna(row['popularity']) else None,
                members=row['members'] if 'members' in row and not pd.isna(row['members']) else None,
